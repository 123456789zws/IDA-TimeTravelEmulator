...
class LightPatchEmuState(EmuState):
    def __init__(self, state_id: str, instruction_address: int = -1, execution_count: int = -1) -> None:
        ...
        self.reg_patches: Dict[str, int] = {}
        self.mem_patches: List[Tuple[int, int, int]] = []
    ...
    def apply_memory_patch(self, address: int, page_size: int) -> None:
        pass
        #TODO
...
class EmuStateManager():
    ...
    def determine_next_state_type(self):
        if self.last_full_state_id is None:
    ...
    def generate_full_state(self, state_id: str) -> Optional[EmuState]:
        pass
    #     """
    #     Generate a full state by traversing the patch chain and applying all the patches.

    #     :param state_id: The ID of the state to generate.
    #     :return: The generated EmuState object if generation is successful, otherwise None.
    #     """

    #     if state_id not in self.states_dict:
    #         tte_log_warn(f"Generate full State: Cannot generate full state for {state_id}: state not found.")
    #         return None

    #     target_state: EmuState = deepcopy(self.states_dict[state_id]) # type: ignore

    #     if target_state.type == EmuState.STATE_TYPE_FULL:
    #         ## The target state is already a full state, return it directly
    #         return target_state

    #     ## Traverse the patch chain and apply all the patches

    #     base_full_state = self.get_state(target_state.base_full_state_id)
    #     if base_full_state is None:
    #         tte_log_warn(f"Generate full State: Cannot generate full state for {state_id}: base full state not found.")
    #         return None

    #     prev_state: Optional[EmuState] = None
    #     while target_state.prev_state_id is not None:
    #         prev_state = self.get_state(target_state.prev_state_id)
    #         if prev_state is None:
    #             tte_log_warn(f"Generate full State: Cannot generate full state for {state_id}: previous state not found.")
    #             return None

    #         if prev_state.type == EmuState.STATE_TYPE_FULL:
    #             # The previous state is already a full state, break the loop
    #             break

    #         ## collect the memory patches from the previous state
    #         # After connecting the patches from older state to the array,
    #         # the patches are performed in the order from back to front when applying them to the memory pages.
    #         target_state.mem_patches.extend(prev_state.mem_patches)

    #         target_state.prev_state_id = prev_state.prev_state_id

    #     ## Generate register values
    #     target_state.registers_map = apply_patch_dict(base_full_state.registers_map, target_state.reg_patches)

    #     ## Generate memory values
    #     # Load the memory pages of the base full state
    #     target_state.memory_pages = base_full_state.memory_pages.copy()

    #     # Apply the memory patches
    #     while target_state.mem_patches != []:
    #         addr, size, value = target_state.mem_patches.pop()
...
class EmuTracer():
    ...
    def cb_create_emu_state(self, uc, address, size, user_data) -> bool:
        ...
        self.state_buffer.mem_patches.clear()
